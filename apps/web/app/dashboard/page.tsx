...
function TiltCard({ children, className, onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) {
  ...
  return (
    <div className="perspective-1000 h-full" onMouseMove={handleMouse} onMouseLeave={handleLeave}>
      <motion.div
        ref={ref}
        style={{ rotateX, rotateY }}
        onClick={onClick}
        className={cn('bento-card preserve-3d h-full', className)}
      >
        {children}
      </motion.div>
    </div>
...
// Destinos em alta — curadoria movida para `lib/destinations.ts` para que
// possamos recomendar destinos locais do país de acesso do usuário.
...
export default function DashboardPage() {
  ...
  // País detectado pela Vercel (header x-vercel-ip-country) → usado para
  // priorizar destinos locais na seção "Em alta".
  const userCountry = useUserCountry('BR');
  const localCountryName = resolveCountryName(userCountry);
    ...
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 space-y-6">
      ...
      {trips.length > 0 ? (
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:grid-rows-[220px_180px]"
        >
          {/* ── Card 1: Próxima Viagem — full-width mobile, 2×2 desktop ── */}
          <motion.div variants={stagger.item} className="col-span-2 lg:row-span-2 min-h-0">
        ...
        {trips.length > 0 && (
          <motion.div variants={stagger.item} className="col-span-1 row-span-1 min-h-0">
        ...
        {trips.length > 0 && (
          <motion.div variants={stagger.item} className="col-span-2 lg:col-span-2 row-span-1 min-h-0">
      ...
      >
        ...
        <TrendingFilterChips
          ...
          localLabel={localCountryName}
        />

        {TRENDING.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-muted/40 p-6 text-center">
            <p className="text-sm text-muted-foreground">
              Ainda não temos destinos curados em {localCountryName}.{' '}
              <button
                type="button"
                onClick={() => setTrendingFilter('international')}
                className="text-primary font-medium hover:underline"
              >
                Ver destinos internacionais
              </button>
            </p>
          </div>
        ) : (
...
// Label "Destinos em X" se adapta dinamicamente ao país detectado pela Vercel,
// então um usuário em Portugal vê "Destinos em Portugal", e por aí vai.

function TrendingFilterChips({
  value,
  onChange,
  localLabel,
}: {
  value: TrendingFilter;
  onChange: (v: TrendingFilter) => void;
  localLabel: string;
}) {
  const chips: Array<{ key: TrendingFilter; label: string }> = [
    { key: 'all', label: 'Todos' },
    { key: 'local', label: `Destinos em ${localLabel}` },
