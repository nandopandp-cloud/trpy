...
const FEATURES = [
  {
    ...
    color: 'from-blue-500 to-violet-500', glow: 'glow-blue',
  },
  {
    icon: Wallet, label: 'Controle financeiro',
    desc: 'Acompanhe gastos por categoria com gráficos em tempo real.',
    color: 'from-emerald-500 to-teal-500', glow: 'glow-green',
  },
  {
    icon: Sparkles, label: 'IA generativa',
    desc: 'Descreva seu destino e a IA cria um itinerário personalizado.',
    color: 'from-amber-400 to-orange-500', glow: 'glow-amber',
  },
  {
    icon: Calendar, label: 'Planejamento fácil',
    desc: 'Datas, orçamento, notas e companheiros — tudo em um só lugar.',
    color: 'from-rose-500 to-pink-500', glow: 'glow-violet',
  },
];

const FLOATING_CARDS = [
  { emoji: '🗼', place: 'Paris', days: '7 dias', color: 'from-blue-600 to-violet-700', delay: 0, rotate: '-3deg' },
  { emoji: '🌊', place: 'Bali', days: '10 dias', color: 'from-emerald-600 to-teal-700', delay: 0.5, rotate: '0deg' },
  { emoji: '🏔️', place: 'Patagônia', days: '14 dias', color: 'from-rose-600 to-pink-700', delay: 1, rotate: '3deg' },
];
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-16 px-6 flex items-center justify-between glass-card border-b">
        <div className="flex items-center gap-2.5">
          <motion.div
            ...
            className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center glow-blue"
          >
            <Map className="w-3.5 h-3.5 text-white" />
          </motion.div>
          <span className="font-bold text-gradient text-lg">Trpy</span>
        </div>
      ...
      <section className="relative pt-36 pb-24 px-6 flex flex-col items-center text-center overflow-hidden">
        ...
        <div className="absolute top-24 left-1/4 w-72 h-72 rounded-full bg-blue-500/10 blur-3xl animate-float pointer-events-none" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full bg-violet-500/10 blur-3xl animate-float-slow pointer-events-none" />
        ...
        >
          ...
          <motion.h1 variants={stagger.item} className="text-5xl md:text-7xl font-black leading-[1.05] tracking-tight">
            Viaje mais,
            <br />
            <span className="text-gradient">planeje melhor.</span>
          </motion.h1>
          ...
          <motion.div variants={stagger.item} className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
            <Link href="/dashboard">
              <Button size="lg" className="gap-2 text-base px-8 glow-blue animate-pulse-glow w-full sm:w-auto">
                Começar grátis <ArrowRight className="w-4 h-4" />
              </Button>
            </Link>
            <Link href="/dashboard/ai">
              <Button size="lg" variant="outline" className="gap-2 text-base px-8 w-full sm:w-auto">
                <Sparkles className="w-4 h-4" /> Ver IA em ação
              </Button>
            </Link>
          </motion.div>
          ...
          <motion.div variants={stagger.item} className="flex items-center justify-center gap-3 text-sm text-muted-foreground">
            <div className="flex -space-x-2">
              {['🧑‍🦱', '👩‍🦰', '🧔', '👩'].map((e, i) => (
                <div key={i} className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-violet-500 border-2 border-background flex items-center justify-center text-xs">
                  {e}
                </div>
              ))}
            </div>
        ...
        <div className="relative mt-16 w-full max-w-3xl mx-auto h-48 hidden md:block">
          {FLOATING_CARDS.map((card, i) => (
            <motion.div
              ...
              className={`absolute glass-card rounded-2xl p-3 w-36 border-gradient shadow-2xl
                ${i === 0 ? 'left-4 top-0' : ''}
            ...
            >
              <div className={`w-full h-16 rounded-xl bg-gradient-to-br ${card.color} flex items-end p-2 mb-2`}>
            ...
            {FEATURES.map((f) => {
              ...
              return (
                <motion.div
                  ...
                  className="relative group rounded-2xl border border-border bg-card p-7 overflow-hidden"
                >
                  <div className={`absolute -top-10 -right-10 w-40 h-40 rounded-full bg-gradient-to-br ${f.color} opacity-0 group-hover:opacity-8 transition-all duration-500 blur-2xl`} />
                  <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center mb-5 ${f.glow}`}>
      ...
      <section className="py-24 px-6">
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="relative max-w-2xl mx-auto text-center overflow-hidden rounded-3xl border border-primary/25 p-14"
        >
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/8 via-violet-500/8 to-purple-500/8" />
          ...
          <div className="relative space-y-5">
            ...
            <h2 className="text-4xl font-black leading-tight">
              Pronta para sua{' '}
              <span className="text-gradient">próxima aventura?</span>
            </h2>
            <p className="text-muted-foreground">Grátis, bonito e fácil de usar.</p>
            <Link href="/dashboard">
              <Button size="lg" className="gap-2 text-base px-10 glow-blue mt-2">
